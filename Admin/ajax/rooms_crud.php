...
if (isset($_POST['add_room'])) {
    ...
    if ($stmt = mysqli_prepare($con, $q2)) {
        ...
        $flag = 0;
        die('Insert Query can not be prepared !! 😢😢');
    }
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        $data .= "
          <tr class='align-middle'>
            ...
            <td>$status</td>
            <td>
              <button type='button' onclick='edit_details($row[id])' class='btn btn-warning shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#edit-room'>
...
if (isset($_POST['get_room'])) {
    ...
    $val1 = [$frm_data['get_room']];
    $res1 = select($q1, $val1, 'i');

    $q2 = "SELECT * FROM `room_features` WHERE `room_id`=?";
    $val2 = [$frm_data['get_room']];
    $res2 = select($q2, $val2, 'i');

    $q3 = "SELECT * FROM `room_facilities` WHERE `room_id`=?";
    $val3 = [$frm_data['get_room']];
    $res3 = select($q3, $val3, 'i');
...
// change room status activate/inactivate
...
if (isset($_POST['remove_room'])){
  ...
  while($row = mysqli_fetch_assoc($res1)){
    deleteImg($row['image'],ROOM_IMG_UPLOAD_FOLDER);
  }
  ...
  // $res6 = delete("DELETE FROM `room_category` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
...
if (isset($_POST['add_image'])) {
    ...
    $img_r = uploadImages($_FILES['image'], ROOM_IMG_UPLOAD_FOLDER);
...
if (isset($_POST['get_room_images'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        echo <<<data
          <tr class='align-middle'>
            <td>$i</td>
            <td><img src='$path$row[image]' width='55%'></td>
            <td>$thumb_btn</td>
            <td>
                <button onclick='delete_room_image($row[sr_no],$row[room_id])' class='btn btn-danger shadow-none'><i class='bi bi-trash'></i></button>
            </td>
          </tr>
        data;
        $i++;
...
if (isset($_POST['delete_room_image'])) {
    ...
    $Q1 = "SELECT * FROM `room_image` WHERE `sr_no`=? AND `room_id`=?";
    $res = select($Q1, $values, 'ii');
    $img = mysqli_fetch_assoc($res);

    if (deleteImg($img['image'], ROOM_IMG_UPLOAD_FOLDER)) {
